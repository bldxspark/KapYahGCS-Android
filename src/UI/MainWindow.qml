...
ApplicationWindow {
    ...
    // Temporary local administrator lock for Vehicle Configuration.
    // Change this value before creating the release build.
    readonly property string _vehicleConfigPassword: "KapYahGCS@2026"
    property bool _vehicleConfigUnlocked: false
    property string _pendingVehicleConfigPage: ""
    ...
    function requestVehicleConfig(page = "") {
        _pendingVehicleConfigPage = page
        if (_vehicleConfigUnlocked) {
            _openPendingVehicleConfig()
        } else {
            vehicleConfigPasswordDialogFactory.open()
        }
    }

    function _openPendingVehicleConfig() {
        closeIndicatorDrawer()
        if (_pendingVehicleConfigPage === "parameters") {
            showVehicleConfigParametersPage()
        } else {
            showVehicleConfig()
        }
        _pendingVehicleConfigPage = ""
    }
    ...
    QGCPopupDialogFactory {
        id: vehicleConfigPasswordDialogFactory
        dialogComponent: vehicleConfigPasswordDialogComponent
    }

    Component {
        id: vehicleConfigPasswordDialogComponent

        QGCPopupDialog {
            id: passwordDialog
            title: qsTr("Administrator Access")
            ...
            onAccepted: {
                if (passwordField.text === mainWindow._vehicleConfigPassword) {
                    mainWindow._vehicleConfigUnlocked = true
                    mainWindow._openPendingVehicleConfig()
                } else {
                    preventClose = true
            ...
            ColumnLayout {
                ...
                QGCLabel {
                    ...
                    text: qsTr("Enter the administrator password to open Vehicle Configuration.")
                ...
                QGCTextField {
                    ...
                    onTextChanged: passwordDialog.passwordIncorrect = false
                }

                QGCLabel {
                    Layout.fillWidth: true
                    visible: passwordDialog.passwordIncorrect
