...
macro(_gst_configure_pkg_config)
    ...
    # GStreamer 1.28.1 for Android ships libpng16.pc but the static OpenGL
    # plugin requests the module name "libpng". Provide a build-local alias so
    # pkg-config can resolve gstopengl without modifying the downloaded SDK.
    if(ANDROID
       AND EXISTS "${GSTREAMER_LIB_PATH}/pkgconfig/libpng16.pc"
       AND NOT EXISTS "${GSTREAMER_LIB_PATH}/pkgconfig/libpng.pc")
        set(_gst_pkgconfig_compat_dir "${CMAKE_BINARY_DIR}/gstreamer-pkgconfig-compat")
        file(MAKE_DIRECTORY "${_gst_pkgconfig_compat_dir}")
        file(WRITE "${_gst_pkgconfig_compat_dir}/libpng.pc"
            "Name: libpng\n"
            "Description: Compatibility alias for the Android GStreamer SDK\n"
            "Version: 1.6.55\n"
            "Requires: libpng16\n")
        list(PREPEND _GPC_LIBDIR "${_gst_pkgconfig_compat_dir}")
    endif()
    ...
    unset(_GPC_LIBDIR)
    unset(_gst_pkgconfig_compat_dir)
